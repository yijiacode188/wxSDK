...
import (
	...
	"github.com/yijiacode188/wxSDK/subscription/handler/openPocAi/vo"
...
// AddVoiceToReCoForText 上传语音文件
// 本接口用于上传语音文件进行转文字识别
// https://developers.weixin.qq.com/doc/service/api/openpoc/ai/api_addvoicetorecofortext.html
func (wx *OpenPocAi) AddVoiceToReCoForText(voiceId string, lang *consts.TranslateLang, fileHeader *multipart.FileHeader) (*vo.AddVoiceToReCoForTextResponse, *utils.Response, error) {
	...
	result, response, err := utils.HttpPost[vo.AddVoiceToReCoForTextResponse](&utils.RequestParams{
...
func CreateMultipartFormData(field string, fileHeader *multipart.FileHeader, fieldHandler ...func(writer *multipart.Writer) error) ([]byte, string, error) {
	body := &bytes.Buffer{}
	writer := multipart.NewWriter(body)
	// 处理其他表单字段（如果提供了回调函数）
	if len(fieldHandler) > 0 && fieldHandler[0] != nil {
		if err := fieldHandler[0](writer); err != nil {
			return nil, "", fmt.Errorf("处理表单字段失败: %v", err)
		}
	}
	// 打开文件
