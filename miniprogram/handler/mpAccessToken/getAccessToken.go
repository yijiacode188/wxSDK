...
func (mp *MPAccessToken) GetAccessToken() (string, *utils.Response, error) {
	...
	err = mp.Store.SetStore(ACCESS_TOKEN, result.AccessToken, time.Now().Add(time.Duration(result.ExpiresIn)*time.Second))
	if err != nil {
		return "", response, err
	}
